class UsersController < SubdomainBaseController
  ...
  private
    ...
    def slack_user_params
      params.require(:user).permit(:first_name, :last_name, :avatar, :reset_avatar)
    end

    def email_user_params
      params.require(:user).permit(:first_name, :last_name, :avatar, :reset_avatar, :email, :password, :password_confirmation)
    end
end
